...
const explicitAllowedOrigins = [
  ...
  'http://localhost:3001',
  'http://localhost:3000',
  'http://localhost:5001' // Add unified deployment URL
].filter(Boolean);
...
app.get('/api/health', (req, res) => {
  res.json({ message: 'Server is running!', deployment: 'unified' });
});
...
if (process.env.NODE_ENV === 'production') {
  ...
  app.get('/', (req, res) => {
    res.json({ 
      message: 'Ticket Management System Backend (Development Mode)',
      status: 'running',
      timestamp: new Date().toISOString(),
      endpoints: {
        health: '/api/health',
...
app.listen(PORT, () => {
  ...
  if (process.env.NODE_ENV === 'production') {
    console.log(`🌐 Unified deployment: http://localhost:${PORT}`);
    console.log(`📱 Frontend + Backend: http://localhost:${PORT}`);
  } else {
    console.log(`🔧 Development mode - Frontend should run on port 3000`);
  }
});
